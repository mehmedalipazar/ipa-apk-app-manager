...
CORS_ORIGINS=                 # üretimde BOŞ bırakın (aşağıda)
```
...
Arayüz `http://localhost:5173`, API `http://localhost:3000` (host portları `.env` içinde
`WEB_PORT` / `API_PORT` ile değişir; container-içi portlar sabittir).

- `ADMIN_PASSWORD` veya `SESSION_SECRET` boşsa compose **başlamaz** (`${VAR:?}`) —
  şifresiz panel sessizce ayağa kalkamaz.
- Kök `.env` yalnızca sırları ve makineye özel değerleri taşır. Üretim varsayılanları
  imaja gömülü `backend/.env.production` içindedir (sırsız). `NODE_ENV`, `PORT` ve
  `DATA_DIR` compose'a bilerek yazılmamıştır — tek kaynakları imajdır; kök `.env`e
...
- **`CORS_ORIGINS` boş kalmalı.** Aynı origin'de CORS gereksizdir; doldurmak oturum
  çerezini `SameSite=None`'a düşürür. Boşken çerez `SameSite=Lax; Secure` olur.
...
Her şey **`./data-docker/`** altındadır (SQLite + IPA dosyaları). Adlandırılmış volume
değil bind mount — sıradan bir dizin gibi yedeklenir:
...
cp frontend/.env.local.example frontend/.env.local    # arayüzü yerel API'ye bağlar

npm run dev:backend      # API  :3000  (veriler backend/data/ içine)
npm run dev:frontend     # arayüz :5173
npm run typecheck        # iki paket birden
...
| `frontend/.env.development` | `https://ipa-ios.simurgbilisim.com` | üretim API'siyle test |
| `frontend/.env.local` | `http://localhost:3000` | yerel backend (satır açıkken diğerlerini ezer) |
...
kabuk. Sırlar yalnızca `backend/.env.local` (dev) ve kök `.env` (compose) içindedir;
`docker compose` sadece kök `.env`'i okur.

Geliştirmede arayüz (5173) ile API (3000) ayrı origin olduğundan
`backend/.env.development` içinde `CORS_ORIGINS=http://localhost:5173` tanımlıdır;
üretimde boştur. Vite `strictPort` ile çalışır: 5173 doluysa sessizce kaymak yerine hata
verir (kayan port CORS listesiyle eşleşmez).

> **Port 3000 çakışması macOS'ta sessizdir.** Dev backend ile api container'ı aynı anda
> dinliyor görünebilir ve istekler hangisine gittiği belli olmaz — üretim trafiği yanlış
> sürece düşebilir. Şüphede: `lsof -nP -iTCP:3000 -sTCP:LISTEN`, gerekirse
> `docker compose stop api`.
...
```bash
docker compose up -d            # B grubu çalışan yığını kullanır
node tests/run-suite.mjs        # tüm gruplar; D canlı HTTPS domain'ini hedefler
node tests/run-suite.mjs A C    # grup seçimi
```

Son tam koşum: **154/154** (10 Ağustos 2026, aynı gün build edilen imajlarla). Senaryo
matrisi `tests/TEST-PLAN.md`, kanıt geçmişi `tests/BULGULAR-HTTPS.md`, ham raporlar
`tests/reports/`.
...
- **Şifre sıfırlama.** `ADMIN_PASSWORD` yalnızca ilk açılışta okunur (panelden
  değiştirilen şifre kalıcı olsun diye). Unutulursa `.env`'e yeni şifreyi yazıp bir kez
  `ADMIN_PASSWORD_FORCE_RESET=true` ile başlatın, sonra değişkeni kaldırın.
- **`SESSION_SECRET` değişirse** tüm oturumlar **ve dağıtılmış tüm imzalı linkler**
  geçersiz olur.
- **Canlı domain 502 veriyorsa** sorun sertifika değil, compose yığınının kapalı
  olmasıdır — `docker compose ps` ve `GET /healthz` ile bakın.
- **Log:** `docker compose logs -f api` (arayüz için `web`). **Sağlık:** iki serviste de
  `GET /healthz`.
